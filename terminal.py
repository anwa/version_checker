...
def find_local_file(app_folder):
    for local_file in os.listdir(app_folder):
        if local_file.startswith("Microsoft.WindowsTerminal_") and local_file.endswith(
            ".msixbundle"
        ):
            return local_file
    return None
...
def terminal(app_folder):
    # Check the current directory for a Microsoft Terminal Installer
    file_to_delete = None
