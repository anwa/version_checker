...
def extract_version_from_filename(filename):
    match = re.search(r"TortoiseGit-(\d+\.\d+\.\d+\.\d+)-64bit\.msi", filename)
...
def find_local_file(app_folder):
    for local_file in os.listdir(app_folder):
        if local_file.startswith("TortoiseGit-") and local_file.endswith(".msi"):
            return local_file
    return None
...
def get_latest_stable_version():
    ...
    for link in links:
        if "href" in link.attrs:
            ...
            if href.endswith("64bit.msi"):
                return f"https:{href}"


def tortoise_git(app_folder):
    # Check the current directory for a TortoiseGit installation file
    file_to_delete = None
