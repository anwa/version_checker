...
def extract_version_from_filename(filename):
    match = re.search(r"Bitwarden-Installer-(\d+\.\d+\.\d+)\.exe", filename)
...
def find_local_file(app_folder):
    for local_file in os.listdir(app_folder):
        if local_file.startswith("Bitwarden-Installer-") and local_file.endswith(".exe"):
            return local_file
    return None
...
def get_latest_stable_version():
    ...
    for asset in latest_release_data.get("assets", []):
        if asset["name"].startswith("Bitwarden-Installer") and asset["name"].endswith(".exe"):
            url = asset["browser_download_url"]
            return url
    return None


def bitwarden(app_folder):
